...
- There is a basic PTY-backed workspace shell with process records, but not a
  ...
  command/output chunks are appended to that raw transcript log. The terminal
  panel can search persisted transcript logs and restores the latest transcript
  after app restart; managed multi-terminal UI, a polished terminal history
