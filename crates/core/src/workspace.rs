...
impl WorkspaceStore {
    ...
    /// Drop a repository registration and all of its workspace records from the
    /// database. This is a metadata-only operation — no worktrees are touched
    /// (used to prune a repository whose root path no longer exists on disk).
    ...
    pub fn create_from_pull_request_with_progress(
        ...
        let workspace = self.create_lifecycle_job_with_progress(
            CreateWorkspace {
                repository_name: repository_name.to_owned(),
                // Empty name → autogenerated linux/distro codename.
                name: workspace_name.map(str::to_owned).unwrap_or_default(),
                branch: branch_name
                    .map(str::to_owned)
                    .unwrap_or_else(|| format!("{prefix}/gh-pr-{pr_number}")),
...
"
    ...
    /// Save a large pasted blob into the thread's workspace worktree under
    /// `.context/archductor/{thread_id}/` and return the saved attachment (its
    /// repo-relative path + token). Used by the composer's paste-to-file flow.
    pub fn save_thread_paste_attachment(
        &self,
        thread_id: i64,
        text: &str,
        ...
        crate::chat_attachments::save_chat_attachment(
            Path::new(&root),
            &thread_id.to_string(),
    ...
    /// Reorder a queued chat input up or down one slot. Queue order is by row id,
    /// so rather than reassign ids we swap the payload of the entry with its
    /// neighbour. Returns the thread id when a swap happened, or None at a
    /// boundary / unknown id (so the caller can skip the broadcast).
    pub fn move_queued_chat_input(&self, queue_id: i64, up: bool) -> Result<Option<i64>> {
        let Some(record) = self.get_queued_chat_input_optional(queue_id)? else {
            return Ok(None);
        };
        let thread_id = record.thread_id;
        let list = self.list_queued_chat_inputs(thread_id)?;
        let Some(pos) = list.iter().position(|r| r.id == queue_id) else {
            return Ok(None);
        };
        let neighbor = if up {
            if pos == 0 {
    ...
    fn write_queued_payload(
        &self,
        target_id: i64,
        src: &QueuedChatInputRecord,
        now: &str,
    ) -> Result<()> {
        self.conn.execute(
            "UPDATE chat_queued_inputs
             SET input = ?1, visible_input = ?2, input_kind = ?3, session_kind = ?4, updated_at = ?5
             WHERE id = ?6",
            params![
                &src.input,
                src.visible_input.as_deref(),
                archcar_input_kind_as_str(src.input_kind.clone()),
                session_kind_as_str(src.session_kind),
                now,
                target_id
            ],
        )?;
...
mod tests {
    ...
    #[test]
    fn remove_repository_drops_registration_and_workspaces() {
        let temp = tempfile::tempdir().unwrap();
        let repo_path = init_repo(temp.path().join("demo"));
        let db_path = temp.path().join("state.db");
        ...
        // Repository row and its workspace records are gone.
        let repos: i64 = store
            .conn
...
"#,
    ...
    #[test]
    fn move_queued_chat_input_reorders_by_swapping_payload() {
        ...
        let order = |s: &WorkspaceStore| -> Vec<String> {
            s.list_queued_chat_inputs(thread.id)
                .unwrap()
                .into_iter()
                .map(|r| r.input)
                .collect()
        ...
        store.move_queued_chat_input(a.id, false).unwrap();
        assert_eq!(order(&store), vec!["c", "a", "b"]);

        // Front item up + unknown id are no-ops.
    ...
    #[test]
    fn save_thread_paste_attachment_writes_file_under_workspace() {
        let (_temp, store) = test_workspace_store();
        let thread = store
            .create_chat_thread("berlin", "codex", "New Chat", None)
            .unwrap();
        let saved = store
            .save_thread_paste_attachment(thread.id, "a very long pasted blob")
            .unwrap();
        assert!(saved
            .relative_path
            .starts_with(&format!(".context/archductor/{}/pasted-text-", thread.id)));
...
"#,
        ...
        // Name is an autogenerated linux/distro codename, not derived from the issue.
        assert!(WORKSPACE_CITY_NAMES.contains(&workspace.name.as_str()));
        assert_eq!(workspace.branch, "lc/gh-issue-123");
        let brief = fs::read_to_string(workspace.path.join(".context/brief.md")).unwrap();
...
"#,
        ...
        restore_path(old_path);
        assert_eq!(workspace.branch, "team/gh-issue-123");
    }
...
"#,
        ...
        restore_path(old_path);
        // Name is an autogenerated linux/distro codename, not derived from the PR.
        assert!(WORKSPACE_CITY_NAMES.contains(&workspace.name.as_str()));
        assert_eq!(workspace.branch, "lc/gh-pr-42");
        assert!(workspace.path.join("pr.txt").exists());
        let pr = store.pull_request(&workspace.name).unwrap().unwrap();
...
"#,
    ...
    #[test]
    fn workspace_city_name_pool_is_curated_open_source_chaos() {
        for expected in [
            "arch",
            "debian",
            "nixos",
            ...
            assert!(
                WORKSPACE_CITY_NAMES.contains(&expected),
                "missing open-source name {expected}"
            );
        }
        for old_city in ["berlin", "tokyo", "paris"] {
            assert!(
                !WORKSPACE_CITY_NAMES.contains(&old_city),
                "old city name {old_city} remains"
            );
        }
    }

    #[test]
    fn workspace_city_name_pool_is_unique_and_slug_safe() {
        let unique = WORKSPACE_CITY_NAMES.iter().copied().collect::<HashSet<_>>();
        assert_eq!(unique.len(), WORKSPACE_CITY_NAMES.len());
        for name in WORKSPACE_CITY_NAMES {
...
"#,
    ...
    fn create_workspace_does_not_treat_repository_name_as_selected_city() {
        let temp = tempfile::tempdir().unwrap();
        let selected_city = WORKSPACE_CITY_NAMES[0];
        let repo_one_path = init_repo(temp.path().join(format!("repo-{selected_city}")));
        ...
        repo_store
            .add(AddRepository {
                name: Some(selected_city.to_owned()),
                root_path: repo_one_path,
                default_branch: Some("main".to_owned()),
                remote_name: "origin".to_owned(),
                workspace_parent_path: Some(
                    temp.path().join(format!("workspaces/{selected_city}")),
                ),
            })
            .unwrap();
        repo_store
        ...
        let workspace = store
            .create(CreateWorkspace {
                repository_name: selected_city.to_owned(),
                name: String::new(),
                branch: String::new(),
                base_ref: Some("main".to_owned()),
            })
