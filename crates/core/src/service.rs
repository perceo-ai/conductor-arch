...
const SERVICE_PASSTHROUGH_ENV: &[&str] = &[
    "ARCHDUCTOR_APNS_TEAM_ID",
    "ARCHDUCTOR_APNS_KEY_ID",
    "ARCHDUCTOR_APNS_KEY_PATH",
    "ARCHDUCTOR_APNS_TOPIC",
    "ARCHDUCTOR_APNS_ENV",
    "ARCHDUCTOR_APNS_ENDPOINT",
];
...
pub fn install(
    ...
    let extra_env = service_extra_env();
    ...
    let contents = match manager {
        ...
        ServiceManager::Systemd => systemd_unit(&binary, listen.as_deref(), &path_env, &extra_env),
        ServiceManager::ScheduledTask => {
            windows_launcher_script(&binary, listen.as_deref(), &path_env, &extra_env)
        }
...
fn service_extra_env() -> Vec<(String, String)> {
    SERVICE_PASSTHROUGH_ENV
        .iter()
        .filter_map(|key| {
            std::env::var(key)
                .ok()
...
pub fn launchd_plist(
    ...
    for (key, value) in extra_env {
        environment.push_str(&plist_env_entry(key, value));
    }
    format!(
...
pub fn windows_launcher_script(
    binary: &Path,
    listen: Option<&str>,
    path_env: &str,
    extra_env: &[(String, String)],
...
pub fn systemd_unit(
    ...
    if let Some(addr) = listen {
        environment.push_str(&format!(
            "Environment={}={}\n",
            remote::LISTEN_ENV,
            systemd_env_quote(addr)
        ));
    }
    for (key, value) in extra_env {
        environment.push_str(&format!("Environment={key}={}\n", systemd_env_quote(value)));
    }
    format!(
        "[Unit]\n\
...
fn env_from_unit(contents: &str, key: &str) -> Option<String> {
    for (index, line) in contents.lines().enumerate() {
        ...
        if let Some(rest) = line.trim().strip_prefix(&format!("Environment={key}=")) {
            return Some(unquote_systemd_env_value(rest.trim()));
        }
...
fn unquote_systemd_env_value(value: &str) -> String {
...
mod tests {
    ...
    #[test]
    fn service_units_record_apns_environment_when_supplied() {
        let extra = vec![
            (
                "ARCHDUCTOR_APNS_TEAM_ID".to_owned(),
                "UM8FW24DFH".to_owned(),
            ),
            (
                "ARCHDUCTOR_APNS_KEY_PATH".to_owned(),
                "/Users/dev/APNs Keys/AuthKey.p8".to_owned(),
            ),
        ];
        ...
        let unit = systemd_unit(Path::new("/usr/local/bin/archcar"), None, NO_PATH, &extra);
        let script = windows_launcher_script(Path::new(r"C:\bin\archcar.exe"), None, "", &extra);

        assert_eq!(
            env_from_unit(&plist, "ARCHDUCTOR_APNS_TEAM_ID").as_deref(),
            Some("UM8FW24DFH")
        );
        assert_eq!(
            env_from_unit(&unit, "ARCHDUCTOR_APNS_KEY_PATH").as_deref(),
            Some("/Users/dev/APNs Keys/AuthKey.p8")
        );
        assert!(
            unit.contains(
                "Environment=ARCHDUCTOR_APNS_KEY_PATH=\"/Users/dev/APNs Keys/AuthKey.p8\""
            ),
            "{unit}"
        );
        assert_eq!(
            env_from_unit(&script, "ARCHDUCTOR_APNS_TEAM_ID").as_deref(),
            Some("UM8FW24DFH")
        );
    }
