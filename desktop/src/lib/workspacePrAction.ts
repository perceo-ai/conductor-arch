...
export function deriveWorkspacePrAction(input: WorkspacePrActionInput): WorkspacePrActionState {
  ...
  if (conflicts > 0)
    return { title: "Merge conflicts", cssClass: "ws-pr-status-failed", actionLabel: "View", action: "view" };
  if (check === "failing" || check === "failure" || check === "error")
    return { title: "Checks failing", cssClass: "ws-pr-status-failed", actionLabel: "View", action: "view" };
  if (ahead > 0)
  ...
  if (check === "pending" || check === "running" || check === "queued")
    return { title: "Checks running", cssClass: "ws-pr-status-pending", actionLabel: "View", action: "view" };
  if (behind > 0)
    return { title: "Behind base", cssClass: "ws-pr-status-pending", actionLabel: "View", action: "view" };
