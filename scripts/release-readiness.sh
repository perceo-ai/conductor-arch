...
if [ "$skip_doctor" -eq 0 ]; then
    if [ ! -x ./target/release/archductor ]; then
        echo "error: ./target/release/archductor is missing; run without --skip-tests first" >&2
        exit 1
    fi
    run ./target/release/archductor doctor
fi
